<div class="row">
	<div class="col-lg-12">
		<h3 class="w3-border-bottom">List of Deposit</h3>
		<?php echo $this->Form->create('CashBook', array('action'=>'list_deposit')); ?>
		...
		<table class="table table-bordered">
			<tr class="w3-light-grey">
				...
				<td style="width:20%;">
					<a href="/print/print/cash_type_two/<?php echo 'deposit'; ?>/<?php echo $for_bir; ?>/<?php echo($date['date_from']); ?>/<?php echo($date['date_to']); ?>" target="_blank" class="w3-button w3-dark-grey w3-block w3-round-small"><i class="fa fa-print"></i> PRINT</a>
				</td>
			</tr>
		</table>
		...
		<div class="panel panel-default">
			<div class="panel-heading" id="headerBackgroundColor">
				<a href="/cash_books/add_deposit" class="w3-btn w3-teal w3-round-small">Add Deposit</a>
			</div>
				...
				<table width="100%" class="table table-striped table-bordered table-hover" id="dataTablesUsers">
					...
					<tbody>
					   <?php foreach ($datas as $key => $data): ?>
							...
							<?php $total += $data['CashBook']['deposit']; ?>
							
							<tr>
							    <td><?php echo $data['CashBook']['id']; ?></td>
								<td><?php echo $data['CashBook']['deposit']; ?></td>
								...
								<td class="actions">
									...
									<?php if($data['CashBook']['id'] == $trasaction_counter_for_delete['CashBook']['id']) { ?>
										...
										<button class="w3-button w3-red w3-round-small" data-toggle="modal" data-target="#myModalCashType<?php echo $data['CashBook']['id']; ?>">DELETE</button></td>
...
<div class="modal fade" id="myModalCashType<?php echo $expenses_id; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog">
		<form method="post" action="/cash_books/delete_deposit" >
			
			<input type="hidden" name="data[CashBook][id]" class="w3-input w3-border w3-light-grey" value="<?php echo $data['CashBook']['id']; ?>">
			<input type="hidden" name="data[CashBook][status]" value="deleted_deposit">
			<input type="hidden" name="data[CashBook][action]" value="list_deposit">
			...
			<div class="modal-content">
				<div class="modal-header w3-red">
					...
					<h4 class="modal-title" id="myModalLabel">DELETE DEPOSIT</h4>
				</div>
				<div class="modal-body">
					<div class="form-horizontal">
					   <div class="form-group">
							...
							<div class="col-sm-5">          
								<input type="text" name="data[CashBook][deposit]" class="w3-input w3-border w3-light-grey w3-xxlarge" value="<?php echo $data['CashBook']['deposit']; ?>" autocomplete="off" readonly>
							</div>
					   </div>
					</div>
				</div>
