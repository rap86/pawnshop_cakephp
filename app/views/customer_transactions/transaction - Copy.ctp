<?php 
	...
	foreach($customer_details as $key => $value)
	{
		...
		$transactionDetails['TransactionInOutItem']			= $value['TransactionInOutItem'];
		
		if($value['CustomerTransaction']['status'] == 'pawned')
		{
			$transactionStatus = 'w3-red';
			
			$transactionStatusLeft = 'w3-red';
			$transactionStatusRight = 'w3-red';
			
		} elseif($value['CustomerTransaction']['status'] == 'ua')
		{
			$transactionStatus = 'w3-green';
			
			$transactionStatusLeft = 'w3-green';
			$transactionStatusRight = 'w3-green';
			
		} elseif( $value['CustomerTransaction']['status'] == 'redeemed')
		{
			$transactionStatus = 'w3-blue';
			
			$transactionStatusLeft = 'w3-blue';
			$transactionStatusRight = 'w3-blue';
			
		} elseif( $value['CustomerTransaction']['status'] == 'auctioned')
		{
			$transactionStatus = 'w3-amber';
			
			$transactionStatusLeft = 'w3-amber';
			$transactionStatusRight = 'w3-amber';
			
		} else {
			
			$transactionStatus = 'w3-gray';
			
			$transactionStatusLeft = 'w3-gray';
			$transactionStatusRight = 'w3-gray';
		}
		
		
		if($value['CustomerTransaction']['status'] == 'ua' && $value['CustomerTransaction']['partial_status'] == 'semiua')
		{
			$transactionStatusRight = 'w3-amber';
		}
...
<?php
	...
	if($this->Session->read('Auth.User.roles') == 'admin')
	{
		$forDatePicker = 'datepicker';
		
		// This is for less other amount
		$readOnlyIfRoleIsClerk = '';
		...
		// This is for less other amount
		$readOnlyIfRoleIsClerk = 'readonly';
	}
			...
			<div class="tab-pane fade in active" id="menu2">
				...
				<?php 
					...
					} elseif($keyValue['status'] == 'ua')
					{
						...
						$button_for_less_principal_payment = 'disabled';
						$button_for_in_out_item = 'disabled';
					...
					// ito yung na auctioned na yung item pero may natira
					// example ring,bracelet ang nabili lang ring
					if($keyValue['partial_status'] == 'semiua')
					{
						$button_for_redeem = 'disabled';
						$button_for_reactivate = 'disabled';
					}
				...
				<div class="row">
					<div class="w3-container">
						<div class="w3-half">
							...
							<div class="w3-container w3-border-bottom w3-padding-16 <?php echo $transactionStatusLeft; ?>" id="headerBackgroundColor">
						...
						<div class="w3-half">
							...
							<div class="w3-container w3-border-bottom w3-padding-16 <?php echo $transactionStatusRight; ?>" id="headerBackgroundColor">
								<h4  class="panel-title" style="text-transform:uppercase;">
									<span class="pull-right">
							...
							<div class="table table-responsive">
								<table class="table table-bordered w3-margin-top">
									...
									<tr>
										...
										<td class="w3-light-grey"><?php echo $keyValue['net_amount']; ?></td>
										<td class="text-bold">Karat</td>
										<td class="w3-light-grey"><?php echo $keyValue['karat']; ?></td>
									</tr>
									<tr>
										<td class="text-bold">Net Amount</td>
										<td class="w3-light-grey" id="netAmountRemaining"><?php echo $keyValue['net_amount_duplicate']; ?></td>
									...
									<tr>
										...
										<td class="w3-light-grey" colspan="4"><?php echo $keyValue['details']; ?></td>
									</tr>
								</table>
							</div>
						</div>
				 ...
				 <?php if(!empty($transactionDetails['TransactionInOutItem'])) { ?>
					 <div class="row">
						<div class="col-lg-12">
							<table class="table table-bordered">
								<tr>
									<td colspan="3" class="w3-center w3-light-gray">
										<b>UNDER AUCTION TAKEOUT DETAILS</b>
									</td>
								</tr>
								<tr>
								...
								<?php foreach($transactionDetails['TransactionInOutItem'] as $key_takeout => $value_takeout){ ?>
									<tr>
				 ...
				 <?php } ?>
				  ...
				  <?php 
				  foreach ($transactionDetails['TransactionInterestPayment'] as $keyy => $valuee) 
				  {
					$checkifemptyValue = trim($valuee['partial_interest_amount']);
						if (empty($checkifemptyValue))
							$checkifemptytrue = 0;
						else
							$checkifemptyfalse = 1;
				  }
				  
				  if($checkifemptyfalse == 1) { ?>
					 <div class="panel panel-default">
						<div class="panel-heading">
							<h4 class="panel-title">
								<a class="w3-block w3-center" data-toggle="collapse" data-parent="#accordion" href="#amountAddInPrincipal">
									<b>PARTIAL INTEREST PAYMENT</b>
								</a>
							</h4>
						</div>
						<div id="amountAddInPrincipal" class="panel-collapse collapse">
							<div class="panel-body">
								<table class="table table-bordered">
									
									<tr>
										<td style="width:20%; font-weight:bold;">Payment Date</td>
										<td style="width:20%; font-weight:bold;">PT Number</td>
										<td style="width:20%; font-weight:bold;">Amount added in Principal</td>
										
										<td style="width:20%; font-weight:bold;">Partial Payment</td>
										<td style="width:20%; font-weight:bold;">Details</td>
									</tr>
									<?php 
									$totalPaymentPartial = 0;
									$amountAddInPrincipal = 0;
									foreach($transactionDetails['TransactionInterestPayment'] as $key_interestx => $value_interest_detailsx) { ?>
									

										<?php if(!empty($value_interest_detailsx['partial_interest_amount'])) { ?>
										<?php $totalPaymentPartial += ($value_interest_detailsx['payment_amount'] + $value_interest_detailsx['service_charge'] + $value_interest_detailsx['other_charge_amount']) - $value_interest_detailsx['partial_interest_amount']; ?>
										<?php $amountAddInPrincipal += $value_interest_detailsx['partial_interest_amount']; ?>
										
											<tr>
												<td><?php echo date('M j, Y', strtotime($value_interest_detailsx['payment_date'])); ?></td>
												<td><?php echo $value_interest_detailsx['pt_number']; ?></td>
												<td><?php echo (($value_interest_detailsx['payment_amount'] + $value_interest_detailsx['service_charge'] + $value_interest_detailsx['other_charge_amount']) - $value_interest_detailsx['partial_interest_amount'])?> = <?php echo (($value_interest_detailsx['payment_amount'] + $value_interest_detailsx['service_charge']) - $value_interest_detailsx['partial_interest_amount']); ?> <?php echo "+ ".$value_interest_detailsx['other_charge_amount']?></td>
												
												<td><?php echo $value_interest_detailsx['partial_interest_amount']; ?></td>
												<td><?php echo $value_interest_detailsx['details']; ?> </td>
											</tr>
										<?php } ?>		
									<?php } ?>
									<tr class="info">
										<td colspan="2"><b>Total Partial</b></td>
										<td colspan="1"><b><?php echo $totalPaymentPartial; ?></b></td>
										<td colspan="1"><b><?php echo $amountAddInPrincipal; ?></b></td>
										<td colspan="1"></td>
									</tr>
								</table>
							</div>
						...
						<div class="table-responsive">
							<table class="table w3-striped w3-border" data-count-payment="<?php echo count($transactionDetails['TransactionInterestPayment']); ?>" id="tablePaymentDetails">
									...
									*/
									...
									if(count($transactionDetails['TransactionInterestPayment']) == 0)
									{
										...
										*/
									    ...
									    $this->year  = $interest_date_difference->y; 
										$this->month = $interest_date_difference->m; 
										$this->day   = $interest_date_difference->d;
										*/
									
										$date_today 	= date('Y-m-d');
										$diffs 			= abs(strtotime($date_today) - strtotime($transactionDetails['TransactionInterestPayment'][ count($transactionDetails['TransactionInterestPayment']) - 1]['payment_starting_date']));
										$years		 	= floor($diffs / (365*60*60*24));
									    $months 		= floor(($diffs - $years * 365*60*60*24) / (31*60*60*24));
									    $days 			= floor(($diffs - $years * 365*60*60*24 - $months*31*60*60*24) / (60*60*24));
											...
											*/
											...
											if($keyValue['book_id'] == 3)
											{
												...
												if($this->month <= 1)
												{
													...
													// need to cofirm what is the real process kung dapat ba talagang -1 month interest pag book 3?
												...
												*/
												...
												/* Added Nov 17, 2022 4:00 PM */
												if($this->month <= 1)
												{
													$first_month_interest_book3 = 0;
												} else {
													$first_month_interest_book3 = round((5 / 100) * $keyValue['gross_amount']);
												}

						
												if($keyValue['deduct_first_month'] == 'no') {
													$redeemgrand_charge_amount = ($service_charge + $payment_amount + $keyValue['net_amount_duplicate']);
												} else {
													// $redeemgrand_charge_amount = ($service_charge + $payment_amount + $keyValue['net_amount_duplicate']) - $first_month_interest_book3;
													$redeemgrand_charge_amount = ($service_charge + $payment_amount + $keyValue['net_amount_duplicate']) - ($first_month_interest_book3 + 5 + ($keyValue['doc_stamp_interest'] / 100) * $keyValue['net_amount_duplicate']);
												}
								...
								?>
									...
									<tr>
										<td class="w3-center">
											<span class="w3-badge w3-green" data-toggle="modal" data-target="#myModalDate<?php echo $value_interest_details['id']; ?>" style="cursor:pointer;">
												<?php echo $value_interest_details['pt_number']; ?>
											</span>
										</td>
											<!-- Modal -->
											<div class="modal fade" id="myModalDate<?php echo $value_interest_details['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
												<div class="modal-dialog modal-sm">
													<div class="modal-content">
														...
														<div class="modal-body w3-large">
															...
															<br />
															+ Additional Charges: <?php echo $value_interest_details['other_charge_amount']; ?>
															<br />
															Less Partial Interest: <?php echo $value_interest_details['partial_interest_amount']; ?>
															<br />
															- Discount: <?php echo $value_interest_details['less_charge_amount']; ?>
															<hr />
															Details: <?php echo $value_interest_details['details']; ?>
															<hr />
										...
										<td class="w3-center">
											<?php 
											if(!empty($value_interest_details['partial_interest_amount'])) {
												echo $value_interest_details['partial_interest_amount'];
											} else {
												echo ($value_interest_details['payment_amount'] + $value_interest_details['other_charge_amount'] + $value_interest_details['service_charge']) - $value_interest_details['less_charge_amount'];
											} ?>
										</td>
										<td class="w3-center">
										...
										<td class="w3-center" style="<?php echo $status_unpaid; ?>">
											<?php 
												if($keyValue['status'] == 'redeemed'){
													echo "paid";
												} else {
													echo $value_interest_details['status'];
												}
											?>
										</td>
										<td class="w3-center">
				...
				<div class="panel-body w3-border w3-round-small w3-light-grey">
					...
					<div class="col-lg-2">
						...
						<a class="<?php echo $anchorButtonDisabled; ?> <?php echo $button_for_interest_payment; ?> w3-button w3-block w3-dark-gray w3-round-small" data-toggle="modal" data-target="#myModalPayment" id="btnCreatePtNumber">New PT No.</a>
							...
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body">
									  ...
									  <?php echo $this->Form->hidden('book_id', array('label'=>false, 'value'=>$keyValue['book_id'], 'id'=>'bookIdNumber')); ?>
										  ...
										  <table class="table table-bordered">
												...
												<tr>
													<td>Starting Date</td>
													<td><input type="text" name="data[TransactionInterestPayment][payment_starting_date]" value="<?php echo date("Y-m-d") ?>" class="w3-input w3-border w3-light-grey <?php echo $forDatePicker; ?>" readonly="readonly"></td>
												</tr>
												<tr>
													<td>Due Date</td>
													<td><input type="text" name="data[TransactionInterestPayment][payment_due_date]" value="<?php echo $books_due_date ?>" class="w3-input w3-border w3-light-grey <?php echo $forDatePicker; ?>" readonly="readonly"></td>
												</tr>
										   </table>
									</div>
									<div class="modal-footer">
					...
					<div class="col-lg-2">
						...
						<a id="redeemItem" class="<?php echo $button_for_redeem; ?> <?php echo $paymentDisabled; ?> w3-button w3-block w3-dark-gray w3-round-small" data-toggle="modal" data-target="#myModalTubos" data-date-now="<?php echo date('Y-m-d'); ?>">Redeem</a>
									...
									<div class="modal-content">
										...
										<div class="modal-body">
										   <table class="table table-bordered">
												...
												<tr>
													<td class="w3-text-red" style="font-weight:bold;">Additional Charges</td>
													<td>
														<input name="data[TransactionRedeemItem][other_charge_amount]"  class="w3-input w3-border w3-light-grey" id="redeemOtherChargeAmount" autocomplete="off">
													</td>
												</tr>
												<tr>
													<td class="w3-text-red" style="font-weight:bold;">Discount</td>
													<td>
														<input name="data[TransactionRedeemItem][less_charge_amount]"  class="w3-input w3-border w3-light-grey" id="redeemLessChargeAmount" <?php echo $readOnlyIfRoleIsClerk; ?> autocomplete="off">
													</td>
												</tr>
												<tr>
												...
												<tr>
													<td>Payment Date</td>
													<td>
														<input name="data[TransactionRedeemItem][date_redeemed]" class="w3-input w3-border w3-light-grey <?php echo $forDatePicker; ?>" value="<?php echo date('Y-m-d'); ?>" readonly="readonly">
													</td>
												</tr>
												<tr>
													<td>Payment Time</td>
					 ...
					 <div class="col-lg-2">
						...
						<a class="<?php echo $button_for_ready_for_action; ?> w3-button w3-dark-gray w3-block w3-round-small" data-toggle="modal" data-target="#myModalSetNewPrice">For Auction</a>
					...
					<!-- /.col-lg-2 -->
					 ...
					 <div class="col-lg-2">
						<a class="<?php echo $button_for_reactivate; ?> w3-button w3-dark-gray w3-block w3-round-small" data-toggle="modal" data-target="#myModalReactivate">B2B Pawn</a>
					 ...
					 <div class="col-lg-2">
						...
						<a class="<?php echo $button_for_less_principal_payment; ?> w3-button w3-block w3-dark-gray w3-round-small" data-toggle="modal" data-target="#myModalPrincipal">Less Principal</a>
										...
										<div class="modal-body">
											<table class="table table-bordered">
												...
												<tr>
													<td>Date</td>
													<td>
														<input name="data[TransactionPrincipalPayment][transaction_date]" class="w3-input w3-border w3-light-grey <?php echo $forDatePicker; ?>" value="<?php echo date('Y-m-d'); ?>" readonly="readonly">
													</td>
												</tr>
												<tr>
													<td>Time</td>
					...
					<div class="col-lg-2">
						...
						<a class="<?php echo $button_for_in_out_item; ?> <?php echo $button_for_in_out_item; ?> w3-button w3-block w3-dark-gray w3-round-small" data-toggle="modal" data-target="#myModalINOUT">IN / OUT</a>
						
						<div class="modal fade" id="myModalINOUT" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
							<div class="modal-dialog">
							
								<form method="post" action="/transaction_in_out_items/add" >
								
									<input type="hidden" name="data[TransactionInOutItem][customer_transaction_id]" value="<?php echo $keyValue['id']; ?>">
									<input type="hidden" name="data[TransactionInOutItem][user_id]" value="<?php echo $this->Session->read('Auth.User.id'); ?>">
									<input type="hidden" name="data[TransactionInOutItem][enabled]" value="1">
									<input type="hidden" name="data[TransactionInOutItem][status]" value="takeout">
								
									<div class="modal-content">
										<div class="modal-header <?php echo $transactionStatus; ?>">
											<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
											<h4 class="modal-title" id="myModalLabel">IN / OUT</h4>
										</div>
										<div class="modal-body">
											<table class="table table-bordered">
												<tr>
													...
													<td style="width:70%;">
														<select name="data[TransactionInOutItem][status]" class="w3-input w3-border w3-light-grey w3-xxlarge">
															<option value="takeout">Takeout</option>
															<option value="ua">Takein</option>
														</select>
													</td>
												</tr>
												<tr>
													...
													<td style="width:70%;">
														<textarea name="data[TransactionInOutItem][details]" class="w3-input w3-border w3-light-grey" autocomplete="off" rows="3"></textarea>
													</td>
												</tr>
												<tr>
													<td>Date</td>
													<td>
														<input name="data[TransactionInOutItem][date_transaction]" class="w3-input w3-border w3-light-grey <?php echo $forDatePicker; ?>" value="<?php echo date('Y-m-d'); ?>" readonly="readonly">
													</td>
												</tr>
												<tr>
													<td>Time</td>
													<td>
														<input name="data[TransactionInOutItem][time_transaction]" class="w3-input w3-border w3-light-grey" value="<?php echo date('H:i:s'); ?>" readonly="readonly">
													</td>
												</tr>
											</table>				
										</div>
										<div class="modal-footer">
											<button type="submit" class="w3-button w3-dark-grey w3-round-small pull-left" id="addTaggedSave" style="display:none;">YES</button>
											<div class="w3-button w3-dark-grey w3-round-small pull-left" id="addTagged">Yes</div>
											<button type="button" class="w3-button w3-dark-grey w3-round-small" data-dismiss="modal">NO</button>
										</div>
									</div>
								</form>	
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table class="table table-bordered">
						...
						<tr>
							<td class="w3-text-red" style="font-weight:bold;">Less Partial Interest</td>
							<td>
								<input name="data[TransactionInterestPayment][partial_interest_amount]"  class="w3-input w3-border w3-light-grey" id="lessPartialAmount"  autocomplete="off" <?php echo $readOnlyIfRoleIsClerk; ?>>
							</td>
						</tr>
						<tr>
							<td>Service Charge</td>
						...
						<tr>
							<td class="w3-text-red" style="font-weight:bold;">Additional Charges</td>
							<td>
								<input name="data[TransactionInterestPayment][other_charge_amount]"  class="w3-input w3-border w3-light-grey" id="otherChargeAmount" autocomplete="off">
							</td>
						</tr>
						...
						<tr>
							<td class="w3-text-red" style="font-weight:bold;">Discount</td>
							<td>
								<input name="data[TransactionInterestPayment][less_charge_amount]"  class="w3-input w3-border w3-light-grey" id="lessChargeAmount" <?php echo $readOnlyIfRoleIsClerk; ?> autocomplete="off">
							</td>
						</tr>
						...
						<tr>
							<td>Payment Date</td>
							<td>
								<input name="data[TransactionInterestPayment][payment_date]" class="w3-input w3-border w3-light-grey <?php echo $forDatePicker; ?>" id="" value="<?php echo date('Y-m-d'); ?>" readonly="readonly">
							</td>
						</tr>
						<tr>
							<td>Payment Time</td>
						...
						<tr>
							<td>Details</td>
							<td>
								<input name="data[TransactionInterestPayment][details]" class="w3-input w3-border w3-light-grey">
							</td>
						</tr>
						<tr>
							<td>OR Number</td>
...
$(document).ready(function(){
	...
	// confimration pt number must be unique
	// start confirmation for creating a new pt number
	$('div#createPtNumberConfirmation').click(function() {
		
		$.ajax({
			async: false,
			type: 'POST',
			url: '/transaction_interest_payments/checkif_ptexist',
			data: {
				'data[TransactionInterestPayment][pt_number]': $('input#myPtNumber').val(),
				'data[TransactionInterestPayment][book_id]': $('input#bookIdNumber').val()
			...
			success: function(data) {
				...
				if(data['notempty'] == 1)
				{
					Swal.fire({
					  type: 'error',
					  title: 'PT Number already exist!',
					  text: 'PT Number must unique!'
					});
					
				} else if(data['ptempty'] == 1)
				{
					Swal.fire({
					  type: 'error',
					  title: 'PT Number must not empty!',
					  text: 'PT Number must unique!'
					});
					
					$('input#myPtNumber').val('');
	...
	$('input#priceForAuction, input#textInputPaymentPrincipal, input#myPtNumber').on('keyup blur', function(event) {
		this.value = this.value.replace(/[^0-9]/g,''); 
	});
	
	// for numbers only.
	$('input#otherChargeAmount, input#redeemOtherChargeAmount, input#lessChargeAmount, input#lessPartialAmount, input#redeemLessChargeAmount').on('keyup blur', function(event) {
		this.value = this.value.replace(/[^0-9]/g,''); 
	});
	...
	$('input#otherChargeAmount').on('keyup', function(event) {
		...
		var tot_partial_charge      = Number( $('input#lessPartialAmount').val() );
		var totalGrandIns 		  = (( Number($(this).val() ) + tot_service_charge + tot_payment_interest) - tot_partial_charge) - tot_less_other_amount;
		//alert(otherInts);
	...
	$('input#lessChargeAmount').on('keyup', function(event) {
		...
		var tot_partial_charge      = Number( $('input#lessPartialAmount').val() );
		var totalGrandIns 		  = (( tot_payment_interest + tot_service_charge  + tot_other_charge ) - tot_partial_charge) - Number($(this).val());
		//alert(otherInts);
		$('input#grandChargeAmount').val(totalGrandIns);
	});
	
	$('input#lessPartialAmount').on('keyup', function(event) {
	
		var tot_payment_interest  = Number( $('input#totPaymentAmount').val() );
		var tot_service_charge    = Number( $('input#totServiceCharge').val() );
		var tot_other_charge      = Number( $('input#otherChargeAmount').val() );
		var tot_less_charge      = Number( $('input#lessChargeAmount').val() );
		var totalGrandIns 		  = (( tot_payment_interest + tot_service_charge  + tot_other_charge ) - tot_less_charge) - Number($(this).val());
		//alert(otherInts);
		$('input#grandChargeAmount').val(totalGrandIns);
	});
