				...
				<div class="col-lg-6">
					...
					<table class="table table-bordered">
						...
						<tr>
							...
							<td>
								<input name="data[CustomerTransaction][sangla_date]" class="w3-input w3-border w3-light-grey <?php echo $forDatePicker; ?>" value="<?php echo date('Y-m-d'); ?>" readonly="readonly">
							</td>
						</tr>
						<tr>
							<td>Granted</td>
						...
						<tr>
							<td>With Advance Interest</td><!--para sa special case na book 3 pero hindi na babawasan ng advance interest-->
							<td>
								<?php echo $this->Form->input('deduct_first_month', array('class'=>'w3-input w3-border w3-light-grey', 'options'=>array(""=>"", "no"=>"No", "yes"=>"Yes"), 'label'=>false, 'id'=>'deductFirstMonth', 'style'=>'font-size:30px;', 'disabled'=>'disabled', 'class'=>'w3-grey')); ?>
							</td>
						</tr>
						<tr>
							<td>Gross Amount</td>
...
$(document).ready(function() {
	...
	$('#bookTransaction').change(function() {
		...
		if(thisVal == 3) {
			$('#deductFirstMonth').val('yes');
			<?php if($this->Session->read('Auth.User.roles') == 'admin') { ?>
			$('#deductFirstMonth').removeAttr('disabled').removeClass('w3-grey').addClass('w3-light-grey');
			<?php } else { ?>
			$('#deductFirstMonth').attr('disabled', 'disabled').removeClass('w3-light-grey').addClass('w3-grey');
			<?php } ?>
		} else {
			$('#deductFirstMonth').val('no');
			$('#deductFirstMonth').attr('disabled', 'disabled').removeClass('w3-light-grey').addClass('w3-grey');
		}
	...
	$('#deductFirstMonth').change(function() {
		...
		if(yesorno == "yes") {
			$('#deductFirstMonth').val('yes')
			$('#grantedServiceCharge').val(5);
			
		}
		
		if(yesorno == "no") {
			$('#deductFirstMonth').val('no')
			$('#grantedServiceCharge').val(0);
		}
	});	
	...
	$('#grossAmount').keyup(function() {
		...
		var grossAmountValue 		= Number($(this).val());
