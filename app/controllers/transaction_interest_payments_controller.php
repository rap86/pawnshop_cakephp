...
class TransactionInterestPaymentsController extends AppController {
	...
	function add() {
		...
		if (!empty($this->data)) {
			...
			if ($this->TransactionInterestPayment->save($this->data['TransactionInterestPayment'])) {
				...
				$this->updateIncrementPtNumbers($this->data['TransactionInterestPayment']['book_id'], $this->data['TransactionInterestPayment']['pt_number'] + 1);
	...
	function edit($id = null) {
		...
		if (!empty($this->data)) {
			...
			if ($this->TransactionInterestPayment->save($this->data['TransactionInterestPayment'])) {
				...
				$partial_interest_amont = str_replace(' ', '', $this->data['TransactionInterestPayment']['partial_interest_amount']);
				if(empty($partial_interest_amont)) {
					$grand_interest_amount = $this->data['TransactionInterestPayment']['grand_interest_amount'];
				} else {
					$grand_interest_amount = $partial_interest_amont;
					
					// pag may laman ang partial_interest_amount - minux sa grand_interest_amount at yung matitira yon ang i add sa principal
					$this->partialAmountNotEmptyAddRemainGrandTotalToNet($this->data['TransactionInterestPayment']['customer_transaction_id'], $this->data['TransactionInterestPayment']['grand_interest_amount']);
				}
				
				$starting_balanced  = $this->fundBookAll();
				$ending_balanced    = $this->fundBookAll() + $grand_interest_amount;
				...
				if($this->data['TransactionInterestPayment']['for_bir'] == 1)
				{
					...
					$ending_balanced_bir    = $this->fundBookAllBIR() + $grand_interest_amount;
				...
				$cash_book['CashBook']['interest_amount']					= $grand_interest_amount - $this->serviceChargeAmount();
				...
				$cash_book['CashBook']['date_created'] 						= $this->data['TransactionInterestPayment']['payment_date'];
	...
	function search() {
		...
		$paymentDetails = $this->TransactionInterestPayment->query("
			...
			WHERE
				TransactionInterestPayment.pt_number = '{$this->data['TransactionInterestPayment']['pt_number']}'
	...
	function checkif_ptexist() {
		// $this->log($this->data, 'samplePT');
		
		$this->loadModel('TransactionInterestPayment');
		
		// $resultQuery= $this->TransactionInterestPayment->query("select pt_number from transaction_interest_payments where pt_number = '{$this->data['TransactionInterestPayment']['pt_number']}' ");
		
		$resultQuery= $this->TransactionInterestPayment->query("
			SELECT 
				a.id, a.book_id, b.pt_number
			FROM
				customer_transactions a
					JOIN
				transaction_interest_payments b ON a.id = b.customer_transaction_id
			WHERE
				b.pt_number = '{$this->data['TransactionInterestPayment']['pt_number']}' AND a.book_id = '{$this->data['TransactionInterestPayment']['book_id']}';
		");
		...
		} elseif(trim($this->data['TransactionInterestPayment']['pt_number']) == '')
		{
