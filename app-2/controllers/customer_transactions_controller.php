...
class CustomerTransactionsController extends AppController {
	...
	function transactionStatus($status) {
		...
		$query_result = $this->CustomerTransaction->query("
					select 
						...
						TransactionInterestPayment.payment_due_date,
		...
		if (!empty($this->data)) {
			...
			*/
			...
			if ($this->CustomerTransaction->save($this->data['CustomerTransaction'])) {
				...
				$cash_book['CashBook']['pawned']					= $this->data['CustomerTransaction']['net_amount_duplicate'];
				$cash_book['CashBook']['service_charge'] 			= $this->data['CustomerTransaction']['granted_service_charge'];
	...
	function checkif_orexist() {
		...
		$resultQuery= $this->TransactionInterestPayment->query("select * from transaction_interest_payments where or_number = '{$this->data['TransactionInterestPayment']['or_number']}' ");
